  ...
  >
    <div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-sm">
      <h1 class="text-2xl font-bold text-center text-blue-600 mb-4">
        Enter Your Queen Eight Digit Code
      </h1>
      <input
        v-model="code"
        type="text"
...
<script>
import { doc, runTransaction, increment } from "firebase/firestore"; // Corrected import
    ...
    const submitVote = async () => {
      ...
      try {
        await runTransaction(db, async (transaction) => {
          // Reference the queensRandomCodes document
          const docRef = doc(db, "queensRandomCodes", "uniqueDocumentID");
          ...
          const queensCodes = data.queensCodes || [];

          const codeEntry = queensCodes.find(
          ...
          codeEntry.studentId = props.queenID;
          codeEntry.used = true;

          // Update the document
          transaction.update(docRef, { queensCodes });

          // Increment votes for the queen
          const queenRef = doc(db, "queens", props.queenID);
          transaction.update(queenRef, {
            votes: increment(1),
          });
        });
