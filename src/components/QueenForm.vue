<template>
  <div v-if="visible" class="container mx-auto p-6">
    <h1 class="text-3xl font-bold mb-4">Queen Data</h1>
    ...
    const submitForm = async () => {
      ...
      try {
        ...
        for (const file of files.value) {
          const params = {
            Bucket: "voting-photo",
            Key: `queens/${Date.now()}_${file.name}`,
            Body: file,
        ...
        const docRef = await addDoc(collection(db, "queens"), {
          name: name.value,
          imageUrls,
        ...
        alert("Queen data saved successfully!");
      } catch (error) {
