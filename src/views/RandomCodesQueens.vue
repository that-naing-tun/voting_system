  ...
  <div class="p-8 bg-gray-50 min-h-screen">
    ...
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-blue-700">
        Random Codes for Queen Management
      </h1>
  ...
  setup() {
    ...
    const submitCodeGeneration = async () => {
      ...
      try {
        ...
        const codesDocument = doc(db, "queensRandomCodes", "uniqueDocumentID");
        await setDoc(codesDocument, { queensCodes: codesArray });

        alert("Random codes for Queen generated and stored successfully!");
    ...
    const loadCodesOnce = async () => {
      try {
        ...
        if (docSnap.exists()) {
          codes.value = docSnap.data().queensCodes || [];
        } else {
          console.log("No codes found for Queens in Firestore.");
        }
      } catch (error) {
